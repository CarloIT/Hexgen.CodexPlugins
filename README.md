...
| 插件显示名称 | Hexgen 软件工程规范 |

`software-engineering` 明确表示“软件工程”，便于以后与其他专业领域的插件区分。三个技能分别保留自己的名称与触发范围。
...
技能会按请求匹配，也可显式选择。跨项目安装不表示每条请求都要执行三个技能。项目 `AGENTS.md` 保存项目长期规则；仅将规则放在本插件仓库中，不会自动把它们分发为其他项目的全局指令。
...
市场条目中的 `./plugins/hexgen-software-engineering` 相对于仓库根目录。每个技能目录包含完整的 `SKILL.md`、引用文件、模板及 `agents/openai.yaml`。
...
两条命令依次执行，前一条失败时先解决原因。安装完成后，在 CLI 或桌面端开启新任务，检查三个技能是否来自“Hexgen 软件工程规范”。桌面端与 CLI 应使用同一套本地 Codex 配置；不同设备、Windows 用户或自定义 `CODEX_HOME` 需要分别配置。
...
1. 在 `plugins/hexgen-software-engineering/skills/<技能名>/` 修改源码及受影响的引用或模板。不要直接编辑 Codex 缓存作为团队变更。
2. 调整插件版本，并在 `CHANGELOG.md` 记录行为、适用范围或包装变化。仅文案修正通常递增补丁版本；新增能力递增次版本；破坏既有工作约定的变更需明确说明升级影响。公开版本不复用。
3. 从仓库根目录执行结构校验和差异检查：
...
第一条刷新 Git 插件源快照，第二条安装对应插件版本，随后开启新任务。普通 `git pull` 只更新源码工作目录，不等于更新插件。发布或更新失败时保留错误信息，先确认分支、权限和源地址，不原样反复重装。
...
仓库路径是维护者当前示例，其他机器换成实际仓库根目录。该脚本只在临时配置中试装，最后恢复当前 PowerShell 的原环境变量；测试文件保留在 `$pluginTestConfig` 便于排查，日常 Codex 不会因此安装插件。需要验证实际技能发现时，让隔离的 Codex App Server 执行 `skills/list`，分别查询仓库目录和无关项目目录；应发现三个带 `hexgen-software-engineering:` 前缀且启用的技能，错误列表为空。
...
原先安装在个人 `~/.codex/skills` 下的三个技能可先保留，待插件安装和新任务验证通过后，再禁用旧版本或将它们备份到技能扫描目录之外。只处理这三个技能，保留其他个人技能。迁入仓库不自动安装插件、不停用原技能，也不使当前已运行任务切换版本。

技能启用设置应使用客户端支持的方式；如编辑配置，须以旧技能实际路径为准。不要删除源码仓库，也不要将备份放在另一处仍会被扫描的技能目录。[官方技能发现与配置说明](https://learn.chatgpt.com/docs/build-skills)

## 校验与文件格式

`scripts/校验插件.ps1` 使用 PowerShell 和 .NET 内置功能，无第三方依赖，需要在允许执行本地脚本的 PowerShell 环境中运行。它检查市场与插件标识、版本基本格式、必要字段、三个技能入口与 UI 元数据、本地 Markdown 引用、UTF-8 无 BOM 和文件内行尾一致性。它不实现完整 YAML/schema 校验，也不调用模型、不安装插件、不连接外部服务。
